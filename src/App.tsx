...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    storedNextId != null
      ? setNextId(parseInt(JSON.parse(storedNextId)))
      : setNextId(1)
    if (storedProducts) {
  ...
  const totalPriceQuantity = products
    .reduce(
      (sum, product) =>
        sum + (product.checked ? product.price * product.quantity : 0),
      0
    )
    .toFixed(2)
    ...
    <div className="mx-auto p-4 bg-gray-900 text-white w-screen h-screen overflow-y-auto pb-16">
      <div className="w-full md:w-2/3 lg:w-1/2 xl:w-1/3 flex flex-col m-auto">
        ...
        <div className="grid grid-cols-3 bg-gray-900 p-4 pr-12 fixed bottom-0 m-auto w-full md:w-2/3 lg:w-1/2 xl:w-1/3">
          ...
          <button
            className="text-center text-red-500 border border-red-500 rounded"
            onClick={() => setProducts([])}
          >
            Eliminar todo
          </button>
